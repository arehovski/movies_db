...
def navigation_bar():
    genres = Genre.objects.all().prefetch_related('movie_set').annotate(total=Count("movie")).order_by('-total')
    countries = Country.objects.all().prefetch_related('movie_set').annotate(total=Count('movie')).order_by('-total')[:8]
